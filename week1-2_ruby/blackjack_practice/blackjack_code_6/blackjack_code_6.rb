...
# 21点だったらカードを引かない
if Score.sum(players[1].deal) == 21
  players[1].blackjack
# プレーヤー1（わたし）がバーストしていなかったら、
elsif Score.sum(players[0].deal) < 22
  # プレーヤー2の得点が、プレーヤー1（わたし）の得点以下の間、プレーヤー2は引き続ける
  while Score.sum(players[1].deal) <= Score.sum(players[0].deal)
    players[1].hand(deck.draw)
    players[1].show_card
    players[1].show_score
  end
# プレーヤー1（わたし）がバーストしていたら、
else
  # プレーヤー2は17点以上になるまで引き続ける
  while Score.sum(players[1].deal) < 17
    players[1].hand(deck.draw)
    players[1].show_card
    players[1].show_score
  end
...
# 21点だったらカードを引かない
if Score.sum(players[2].deal) == 21
  players[2].blackjack
else
  # プレーヤー1（わたし）とプレーヤー2がどちらもがバーストしていたら、
  if Score.sum(players[0].deal) > 21 && Score.sum(players[1].deal) > 21
    # プレーヤー3は17点以上になるまで引き続ける
    while Score.sum(players[2].deal) < 17
  ...
  # プレーヤー1（わたし）がバーストしていなかったら、
  if Score.sum(players[0].deal) < 22
    # プレーヤー2の得点が、プレーヤー1（わたし）の得点以下の間、プレーヤー3は引き続ける
    while Score.sum(players[2].deal) <= Score.sum(players[0].deal)
      players[2].hand(deck.draw)
      players[2].show_card
      players[2].show_score
    end
  ...
  # プレーヤー2がバーストしていなかったら
  if Score.sum(players[1].deal) < 22
    # プレーヤー2の得点が、プレーヤー3の得点以下の間、プレーヤー3は引き続ける
    while Score.sum(players[2].deal) <= Score.sum(players[1].deal)
      players[2].hand(deck.draw)
      players[2].show_card
      players[2].show_score
    end
...
if Score.sum(players[0].deal) > 21 && Score.sum(players[1].deal) > 21 && Score.sum(players[2].deal) > 21
  ...
  if max_score_player.size == 1
    ...
    puts "#{max_score_player.last.name}の勝ちです。"
  end
end
