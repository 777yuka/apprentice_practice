...
class Game
  ...
  def initialize
    ...
    @player1 = Player.new('あなた', @deck.draw(2)) # プレイヤー（あなた）を生成する
    @player2 = Player.new('player2', @deck.draw(2)) # プレイヤー（2）を生成する
    @player3 = Player.new('player3', @deck.draw(2)) # プレイヤー（3）を生成する
    @dealer = Player.new('ディーラー', @deck.draw(2)) # プレイヤー（ディーラー）を生成する
  end
  ...
  def other_player_turn(player)
    while Score.sum(player.deal) < 17 # 17点以上になるまでカードを引き続ける
      player.hand(deck.draw)
      show_card(player)
      show_score(player)
    end
  ...
  def judge(players)
    ...
    winner = valid_players.max_by { |player| Score.sum(player.deal) } # 最高得点を持つプレイヤーを代入
    winners = valid_players.select { |player| Score.sum(player.deal) == Score.sum(winner.deal) } # 同点の場合、勝者を複数代入
